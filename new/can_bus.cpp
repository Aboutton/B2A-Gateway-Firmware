...
void setupCAN() {
  ...
  if (gw_config.can1_enabled) {
    ACAN2517FDSettings settings(ACAN2517FDSettings::OSC_40MHz, gw_config.can1_bitrate, DataBitRateFactor::x1);
  ...
  if (gw_config.can2_enabled) {
    ACAN2517FDSettings settings(ACAN2517FDSettings::OSC_40MHz, gw_config.can2_bitrate, DataBitRateFactor::x1);
    settings.mRequestedMode = ACAN2517FDSettings::Normal20B;
  ...
  digitalWrite(CAN1_TERM, gw_config.can1_term ? HIGH : LOW);
  digitalWrite(CAN2_TERM, gw_config.can2_term ? HIGH : LOW);

  Serial.printf("\nCAN Termination: CAN1=%s CAN2=%s\n",
    gw_config.can1_term ? "ON" : "OFF",
    gw_config.can2_term ? "ON" : "OFF");
...
bool canSend(uint8_t bus, uint32_t id, bool ext, const uint8_t* data, uint8_t dlc) {
  ...
  bool success = false;
  if (bus == 1 && gw_config.can1_enabled) {
    success = can1.tryToSend(msg);
    if (success) state.can1_tx_count++;
  } else if (bus == 2 && gw_config.can2_enabled) {
